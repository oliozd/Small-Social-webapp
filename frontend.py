...
def login_page():
  st.title("🚀 Welcome to Small Social")
        ...
        for post in posts:
            ...
            with col2:
                if post.get('is_owner', False):
                    if st.button("🗑️", key=f"delete_{post['post_id']}", help="Delete post"):
                        # Delete the post
                        response = requests.delete(f"http://localhost:8000/feed/{post['post_id']}", headers=get_headers())
                        if response.status_code == 200:
